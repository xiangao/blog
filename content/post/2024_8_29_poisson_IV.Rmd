...
On a side note, the same author has "reghdfe" as a replacement for "areg" in Stata.  "reghdfe" is faster than "areg", or "xtreg, fe".  It also allows for many levels of fixed effects.
...
In the case that the endogenous variable is continous and we can use a linear model on the first stage, Wooldridge 2010 textbook (chapter 18) suggested using control function approach, which is very simple:
...
To verify our control function approach, we do the same with a linear model.  By the way, linear model and Poisson model are the only two models that control functions work, as far as I understand it.  There is a package "ivreghdfe" we use to verify our two step process.
