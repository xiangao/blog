...
The most common distance measure is the propensity score (PS), which is the probability of being treated given $X$.  The benefit is that when there is high dimention of $X$, we can reduce the dimention to a single number, the PS.  
...
Or we can compute distnace from covariates directly, not using propensity score.  The distance can be computed using Mahalanobis distance, or Euclidean distance, or other distance measures.
...
NNM is not optimal in the sense that it does not necessarily minimize the total distance between matched pairs.  Optimal pair matching is a method that minimizes the total distance (or some other overall crietrion) between matched pairs.  It is more computationally intensive than NNM, but it can lead to better balance.
