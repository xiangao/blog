...
We can see in the following example of POLS (which is TWM), we add in "race" which is time-invariant.  It can be estimated in TWM, but would not be estimatible in TWFE.  Notice that coefficients on age and union remain unchanged.  This is basically to say that race is orthogonal to the surface of all the $X$'s and the means of $X$'s.  We did not add in unit invariant variables, but that would be the same story.  
