...
We are usually interested in ATE, the average treatment effect.  However, there could be more complicated situations that the treatment is continuous, or the treatment is multivalued, or the treatment is time-varying.  For example, if we have such a DAG:
...
Suppose treatment $A$ is a function of the history of treatment and confounders, for example, $A = d(A_1, L_1, A_2, L_2)$.  $A$ can be set to a fixed value, say 1 or 0, or some value $A^d$.  This function $d$ can be anything, it can be taking a determinstic value, or it can be a function that takes the natural treatment value $A$ as input.  In the package "lmtp", this is called a shift function, or hypothetical intervention.  For example, $d$ can be set to 1 if $age < 30$, or $d$ can be set to double the natural value of $A$.  Many possibilities.
...
This is basicall g-formula extended to longitudinal data.  
...
```{r sim3, include=TRUE, warning=FALSE,  cache=TRUE, message=FALSE, echo=FALSE}
library(tidyverse)
library(lmtp)
...
```{r sim4, include=TRUE, warning=FALSE,  cache=TRUE, message=FALSE, echo=FALSE}
...
```{r sim5, include=TRUE, warning=FALSE,  cache=TRUE, message=FALSE, echo=FALSE}
library(DynTxRegime)
data(bmiData)
bmi <- bmiData
glimpse(bmi)
...
```{r sim6, include=TRUE, warning=FALSE,  cache=TRUE, message=FALSE, echo=FALSE}
fit_MR <- lmtp_sdr(
  data = bmi, 
  trt = c("A1", "A2"), 
  outcome = "month12BMI", 
...
```{r sim7, include=TRUE, warning=FALSE,  cache=TRUE, message=FALSE, echo=FALSE}
fit_CD <- lmtp_sdr(
  data = bmi, 
  trt = c("A1", "A2"), 
  outcome = "month12BMI", 
...
```{r sim8, include=TRUE, warning=FALSE,  cache=TRUE, message=FALSE, echo=FALSE}
