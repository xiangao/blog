...
gen_data <- function(seed, n, t){
  ...
  a1 <- rbinom(n, 1, plogis(u + x1))  # treatment at time 1
  a2 <- rbinom(n, 1, plogis(u + a1 +  x1))  # treatment at time 2
  y1 <-  u + x1  +  rnorm(n, mean = 0, sd = 1)  # outcome at time 1
  y2 <-  y1 + u + x1 + 2*a2 + rnorm(n, mean = 0, sd = 1)  # outcome at time 2
  data <- data.frame(u=u, x1=x1, x2=x2, a1=a1, a2=a2, y1=y1, y2=y2)
  return(data)
}
data <- gen_data(333, 100000, 2)  
...
Or we can use more sophisticated methods.  Here I use the m-estimation code by Paul Zivich:  https://github.com/pzivich/publications-code/blob/master/ProximalCI/proximal_sims_r.R
...
Both the 2sls and the m-estimation give us the same estimate of the causal effect of treatment on outcome, which is 2.  The m estimator can be more general.  2sls is good for the linear case.
