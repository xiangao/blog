...
Stata's margins command has been a powerful tool for many economists.  It can calculate predicted means as well as predicted marginal effects.  In this post, I'd like to introduce a way to compare effects.
...
For example, we see in the second margins output, the effect of female on probability is .006 at piont 1 (age 20), .0087 at point 2 (age 30), their confidence intervals overlap.  However, if we do a statistical test of the difference, we see the difference is significant.
...
This "mlincom" is a user-written command to test the difference between point 2 and 1.
